...
variable "assign_identity_verifier_role_to_current" {
  type        = bool
  description = "If true, assigns 'Artifact Signing Identity Verifier' on the Artifact Signing account to the identity running terraform apply. This requires permission to create role assignments (for example Owner or User Access Administrator)."
  default     = false
}
